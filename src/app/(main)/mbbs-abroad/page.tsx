...
async function getMBBSAbroadColleges(): Promise<College[]> {
  try {
    console.log('🔍 [MBBS Abroad Page] Connecting to database...')
    await connectDB()
    console.log('🔍 [MBBS Abroad Page] Database connected. Fetching colleges...')
    
    // Fetch all active MBBS abroad colleges directly from MongoDB
    ...
    console.log('🔍 [MBBS Abroad Page] Found MBBS colleges in DB:', collegesData.length)

    // Map Mongoose documents to plain JSON-serializable types expected by client component
    const mbbsColleges: College[] = collegesData.map((college: any) => ({
